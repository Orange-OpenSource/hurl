 ...
 */
use crate::ast::{
    Assert, Comment, CookiePath, Entry, Filter, FilterValue, Header, HurlFile, KeyValue,
    LineTerminator, Method, Predicate, Query, QueryValue, Request, Response, Section, SectionValue,
...
fn walk_colon<V: Visitor>(visitor: &mut V) {
...
fn walk_header<V: Visitor>(visitor: &mut V, header: &Header) {
...
fn walk_request<V: Visitor>(visitor: &mut V, request: &Request) {
    ...
    request.headers.iter().for_each(|h| visitor.visit_header(h));
    request
        .sections
        .iter()
