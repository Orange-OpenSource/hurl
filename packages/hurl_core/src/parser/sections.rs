...
fn section_value_query_params(
    reader: &mut Reader,
    short_syntax: bool,
) -> ParseResult<SectionValue> {
    let params = zero_or_more(key_value, reader)?;
    Ok(SectionValue::QueryParams(QueryParams {
        params,
        short_syntax,
    }))
}
...
fn section_value_form_params(reader: &mut Reader, short_syntax: bool) -> ParseResult<SectionValue> {
    let params = zero_or_more(key_value, reader)?;
    Ok(SectionValue::FormParams(FormParams {
        params,
        short_syntax,
    }))
}
...
fn section_value_cookies(reader: &mut Reader) -> ParseResult<SectionValue> {
    ...
    Ok(SectionValue::Cookies(Cookies(items)))
}

fn section_value_captures(reader: &mut Reader) -> ParseResult<SectionValue> {
    let captures = zero_or_more(capture, reader)?;
    Ok(SectionValue::Captures(Captures(captures)))
}
