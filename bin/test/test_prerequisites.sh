...
function check_listen_port(){
    ...
    if [ -z "${label}" ] || [ -z "${host}" ] || [ -z "${port}" ] ; then
        echo "${color_red}Usage:${color_reset} check_listen_port {label} {host} {port} {command}"
        return 1
    fi

    for count in $(seq 30) ; do
        if nc -zv "${host}" "${port}" ; then
            exit_message="${color_green}$(date) - ${label} listening${color_reset} on ${host}:${port}"
            exit_code=0
            break
        else
            echo "$(date) - ${count} try - ${label} not listening${color_reset} on ${host}:${port} yet"
            exit_message="${color_red}$(date) - ${label} not listening${color_reset} on ${host}:${port}"
            exit_code=1
        fi
        sleep 1
    done
...
if [[ "$(uname -s)" = "Darwin*" ]]; then
  echo -e "\n------------------ Starting server.py (::1)"
  python3 server.py --host "::1" --port 8000 > build/server-ipv6.log 2>&1 &
  check_listen_port "server.py" "::1" 8000 || cat_and_exit_err build/server-ipv6.log
fi
...
check_listen_port "ssl/server.py" "127.0.0.1" 8001 || cat_and_exit_err build/server-ssl-selfsigned.log
...
check_listen_port "ssl/server.py" "127.0.0.1" 8002 || cat_and_exit_err build/server-ssl-signedbyca.log
...
check_listen_port "ssl/server.py" "127.0.0.1" 8003 || cat_and_exit_err build/server-ssl-client-authent.log
