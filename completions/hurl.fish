...
complete -c hurl -l http2-prior-knowledge -d 'Tell Hurl to use HTTP version 2 without HTTP/1.1 Upgrade'
...
complete -c hurl -l jobs -d 'Maximum number of parallel jobs, 1 to disable parallel execution'
...
complete -c hurl -l no-assert -d 'Ignore asserts defined in the Hurl file'
...
complete -c hurl -l no-header -d 'Remove header(s) sent to server'
complete -c hurl -l no-jsonpath-coercion -d 'Disable JSONPath result coercion'
...
complete -c hurl -l proxy-header -d 'Extra header to include in the request when sending to a proxy'
